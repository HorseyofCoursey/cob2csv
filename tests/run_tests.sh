...
"$BIN_DIR/cob2csv" -s "$TEST_DIR/customer.cfg" -i "$TEST_DIR/customer.dat" -o "$OUT" 2>/dev/null
if diff -q "$TEST_DIR/customer_expected.csv" "$OUT" > /dev/null 2>&1; then
    ...
    echo "  Expected:"; cat "$TEST_DIR/customer_expected.csv" | sed 's/^/    /'
...
"$BIN_DIR/csv2cob" -s "$TEST_DIR/customer.cfg" -i "$TEST_DIR/customer_expected.csv" -o "$ROUNDTRIP" 2>/dev/null
...
if "$BIN_DIR/cob2csv" -i "$TEST_DIR/customer.dat" 2>&1 | grep -q "ERROR"; then
...
if "$BIN_DIR/cob2csv" -s "$TEST_DIR/customer.cfg" 2>&1 | grep -q "ERROR"; then
